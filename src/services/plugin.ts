...
class PluginService {
    ...
    getPluginManifest = async (url?: string, useProxy: boolean = false): Promise<LobeChatPluginManifest> => {
        // 1. valid plugin
        if (!url) {
    ...
    private convertOpenAIManifestToLobeManifest = (data: OpenAIPluginManifest): LobeChatPluginManifest => {
