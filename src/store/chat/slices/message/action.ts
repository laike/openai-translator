...
export const chatMessage: StateCreator<ChatStore, [['zustand/devtools', never]], [], ChatMessageAction> = (
    ...
    coreProcessMessage: async (messages, userMessageId) => {
        ...
        const { isFunctionCall, content, functionCallAtEnd, functionCallContent } = await fetchAIChatMessage(
            messages,
            mid
        )
    ...
    fetchAIChatMessage: async (messages, assistantId) => {
        ...
        if (config.model === 'gpt-4-vision-preview') {
            if (!config.params.max_tokens) config.params.max_tokens = GPT4_VISION_MODEL_DEFAULT_MAX_TOKENS
        }
