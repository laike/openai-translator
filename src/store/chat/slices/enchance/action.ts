...
export interface ChatEnhanceAction {
    ...
    ttsMessage: (id: string, state?: { contentMd5?: string; file?: string; voice?: string }) => Promise<void>
...
export const chatEnhance: StateCreator<ChatStore, [['zustand/devtools', never]], [], ChatEnhanceAction> = (
    set,
    get
) => ({
    clearTTS: async (id) => {
    ...
    toggleDallEImageLoading: (key, value) => {
        set({ dalleImageLoading: { ...get().dalleImageLoading, [key]: value } }, false, n('toggleDallEImageLoading'))
    },
