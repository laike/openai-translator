...
const Footer: ShareMessage = {
    from: 'gpt',
    value: `Share from [**🤯 LobeChat**](https://github.com/lobehub/lobe-chat) - ${dayjs().format('YYYY-MM-DD')}`,
}

const PLUGIN_INFO = (plugin: { apiName: string; content: string; identifier: string }): ShareMessage => ({
    from: 'gpt',
    value: [
...
export interface ShareAction {
    shareToShareGPT: (props: { avatar?: string; withPluginInfo?: boolean; withSystemRole?: boolean }) => void
}

export const chatShare: StateCreator<ChatStore, [['zustand/devtools', never]], [], ShareAction> = (set, get) => ({
