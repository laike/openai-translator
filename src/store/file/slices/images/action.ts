...
export const createFileSlice: StateCreator<FileStore, [['zustand/devtools', never]], [], FileAction> = (set, get) => ({
    clearImageList: () => {
    ...
    uploadFile: async (file) => {
        try {
            ...
            set(({ inputFilesList }) => ({ inputFilesList: [...inputFilesList, data.id] }), false, n('uploadFile'))
        } catch (error) {
            // 提示用户上传失败
