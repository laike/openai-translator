...
export const createPluginStoreSlice: StateCreator<ToolStore, [['zustand/devtools', never]], [], PluginStoreAction> = (
    set,
    get
) => ({
    ...
    useFetchInstalledPlugins: () =>
        useSWR<LobeTool[]>(INSTALLED_PLUGINS, pluginService.getInstalledPlugins, {
            onSuccess: (data) => {
                set({ installedPlugins: data, loadingInstallPlugins: false }, false, n('useFetchInstalledPlugins'))
            },
            revalidateOnFocus: false,
        }),
    useFetchPluginStore: () => useSWR<LobeChatPluginsMarketIndex>('loadPluginStore', get().loadPluginStore),
})
