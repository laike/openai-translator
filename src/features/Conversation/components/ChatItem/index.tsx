import { type AlertProps, ChatItem } from '@toboto/lobehub-ui'
...
const Item = memo<ChatListItemProps>(({ index }) => {
    ...
    const item = useChatStore((s) => chatSelectors.currentChatsWithGuideMessage(meta)(s)[index], isEqual)

    const [loading, onMessageChange] = useChatStore((s) => [s.chatLoadingId === item.id, s.updateMessageContent])
    ...
    return (
        <ChatItem
            ...
            renderMessage={(editableContent) => <RenderMessage data={item} editableContent={editableContent} />}
            text={{
                cancel: t('cancel'),
                confirm: t('ok'),
                edit: t('edit'),
