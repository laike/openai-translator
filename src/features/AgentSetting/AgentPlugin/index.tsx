import { Avatar, Form, Icon, Tooltip } from '@toboto/lobehub-ui'
...
const AgentPlugin = memo(() => {
    ...
    const [userEnabledPlugins, toggleAgentPlugin] = useStore(
        (s: { config: { plugins: any }; toggleAgentPlugin: any }) => [s.config.plugins || [], s.toggleAgentPlugin]
    )
    ...
    const list = installedPlugins.map(({ identifier, type, meta, author }) => {
        ...
        return {
            ...
            children: isCustomPlugin ? <LocalPluginItem id={identifier} /> : <PluginAction identifier={identifier} />,
    ...
    const deprecatedList = userEnabledPlugins
        .filter((pluginId: string) => installedPlugins.findIndex((p) => p.identifier === pluginId) < 0)
        .map((id) => ({
