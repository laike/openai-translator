...
const PluginSwitch = memo<{ identifier: string }>(({ identifier }) => {
    ...
    const [userEnabledPlugins, hasPlugin, toggleAgentPlugin] = useStore(
        (s: { config: { plugins: any }; toggleAgentPlugin: any }) => [
            s.config.plugins || [],
            !!s.config.plugins,
            s.toggleAgentPlugin,
        ]
    ...
    return (
        <Flexbox align={'center'} gap={8} horizontal>
            <Switch
                checked={
                    // 如果在加载中，说明激活了
                    pluginManifestLoading[identifier] || !hasPlugin ? false : userEnabledPlugins.includes(identifier)
                }
