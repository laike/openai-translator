...
export const store: StateCreator<Store, [['zustand/devtools', never]]> = (set, get) => ({
    ...
    autocompleteAgentTags: async () => {
        ...
        chatService.fetchPresetTaskResult({
            ...
            params: chainSummaryTags([meta.title, meta.description, systemRole].filter(Boolean).join(',')),
        })
    },
    ...
    autocompleteMeta: (key) => {
        const { autoPickEmoji, autocompleteAgentTitle, autocompleteAgentDescription, autocompleteAgentTags } = get()
