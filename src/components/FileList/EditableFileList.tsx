...
const EditableFileList = memo<EditableFileListProps>(({ items, editable = true, alwaysShowClose, padding = 12 }) => {
    const { mobile } = useResponsive()
    return (
        <Flexbox gap={mobile ? 4 : 6} horizontal padding={padding} style={{ overflow: 'auto', width: '100%' }}>
            <ImageGallery>
                {items.map((i) => (
